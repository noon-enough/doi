...
const dragGraph = function ({x = 30, y = 30, w, h, type, text,
                                fontSize = 60, color = 'black', url = null,
                                rotate = 0, sourceId = null, selected = true},
                            canvas, factor) {
    let that = this
    if (type === 'text') {
        ...
        that.centerX = x + textWidth / 2
        that.centerY = y + textHeight / 2
        that.w = textWidth
        that.h = textHeight
    } else {
        that.centerX = x + w / 2
    ...
    that.square = [
        [that.x, that.y],
        [that.x + that.w, that.y],
        [that.x + that.w, that.y + that.h],
        [that.x, that.y + that.h]
    ];
...
dragGraph.prototype = {
    ...
    paint() {
        ...
        let textHeight = 0;
        if (that.type === 'text') {
            that.ctx.setFontSize(that.fontSize)
            that.ctx.setTextAlign('center')
            that.ctx.setTextBaseline('middle')
            that.ctx.setFillStyle(that.color)
            textWidth = that.ctx.measureText(that.text).width;
            textHeight = that.fontSize + 10;
            // 字体区域中心点不变，左上角位移
            that.x = that.centerX - textWidth / 2;
            that.y = that.centerY - textHeight / 2;
        }
        // 旋转元素
        that.ctx.translate(that.centerX, that.centerY);
        that.ctx.rotate(that.rotate * Math.PI / 180);
        that.ctx.translate(-that.centerX, -that.centerY);
        // 渲染元素
        if (that.type === 'text') {
            that.ctx.fillText(that.text, that.centerX, that.centerY);
        } else if (that.type === 'image') {
            that.ctx.drawImage(that.fileUrl, that.x, that.y, that.w, that.h);
        }
...
Component({
    ...
    attached() {
        ...
        that.factor = screenWidth / 750

        if (typeof that.drawArr === 'undefined') {
            that.drawArr = [];
        }
        that.ctx = wx.createCanvasContext('canvas-label', this);
        that.draw();
    },
    ...
    methods: {
        toPx(rpx) {
            let that = this
            return rpx * that.factor;
        },
        initBg(){
            let that = this
            that.data.bgColor = '';
            that.data.bgSourceId = '';
            that.data.bgImage = '';
        },
        initHistory(){
            this.data.history = [];
        ...
        onGraphChange(n, o) {
            ...
            that.drawArr.push(new dragGraph(Object.assign({
                x: 100,
                y: 70,
            }, n), that.ctx, that.factor))
            that.draw()
            // 参数有变化时记录历史
            that.recordHistory()
        },
        initByArr(newArr) {
            ...
            newArr.forEach((item, index) => {
                switch (item.type) {
                    ...
                    case 'image':
                    case 'text':
                        item.selected = index === newArr.length - 1;
        ...
        draw() {
            let that = this,
                widthToPx = that.toPx(that.data.width),
                heightToPx = that.toPx(that.data.height)

            if (that.data.bgImage !== '') {
                that.ctx.drawImage(that.data.bgImage, 0, 0, widthToPx, heightToPx);
            }
            if (that.data.bgColor !== '') {
                that.ctx.save();
                that.ctx.fillStyle = that.data.bgColor
                // that.ctx.setFillStyle(this.data.bgColor);
                that.ctx.fillRect(0, 0,widthToPx, heightToPx);
                that.ctx.restore();
            }
            that.drawArr.forEach((item) => {
                item.paint();
            });
